...
impl<'r> FromRequest<'r> for GetQueryParams {
    ...
    async fn from_request(request: &'r Request<'_>) -> Outcome<Self, Self::Error> {
        let name = request
            .query_value::<String>("name")
            .and_then(|res| res.ok())
            .unwrap_or_default();
        let password = request
            .query_value::<String>("password")
            .and_then(|res| res.ok());
        let use_root_password = request
            .query_value::<bool>("use_root_password")
            .and_then(|res| res.ok());
        Outcome::Success(GetQueryParams {
            name,
            password,
            use_root_password,
        })
...
#[get("/get")]
pub fn get_data(
    seg_mgmt: &State<Arc<RwLock<DeviceStatus>>>,
