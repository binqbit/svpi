...
impl SegmentManager {
    ...
    pub fn read_values<T: Sized>(&mut self, address: u32, record_direction: RecordDirection) -> std::io::Result<Vec<T>> {
        ...
        let data = match record_direction {
            ...
            RecordDirection::Left => self.spdm.read_data(address - size * value_size, size * value_size)?,
        };
        let mut result = Vec::new();
        let value_size = value_size as usize;
        for i in 0..size as usize {
            let value = unsafe {
                std::ptr::read(data[i * value_size..].as_ptr() as *const T)
            };
            result.push(value);
        }
        Ok(result)
    ...
    pub fn write_values<T: Sized>(&mut self, address: u32, values: &[T], record_direction: RecordDirection) -> std::io::Result<()> {
        ...
        match record_direction {
            ...
            RecordDirection::Left => self.spdm.write_data(address - values.len() as u32 * value_size as u32, &data),
        }
    }
